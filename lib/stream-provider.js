...
async function getMeta(config, type, season, imdbId) {
    const TMDBApiKey = config["TMDBApiKey"]

    if (TMDBApiKey) {
        return await Tmdb.getMeta(type, imdbId, TMDBApiKey, season)
    } else {
...
function getSearchKeys(config, metadataDetails) {
    ...
    return languages.flatMap((language) => {
        const names = metadataDetails['names'].get(language)
        if (names) {
            return names
        }
        return []
...
async function getMovieStreams(config, type, id) {
    const metadataDetails = await getMeta(config, type, null, id)
...
async function getSeriesStreams(config, type, id) {
    ...
    const metadataDetails = await getMeta(config, type, season, imdbId)
    const searchKey = getSearchKeys(config, metadataDetails)
...
function filterSeason(torrent, season) {
    if (torrent && torrent.info.season) {
        torrent.info.season == season
    }
...
function filterEpisode(torrentDetails, season, episode) {
    torrentDetails.videos = torrentDetails.videos
        .filter(video => {
            const video_season = video.info.season || 1;
            return (season == video_season) && (episode == video.info.episode)
        })
