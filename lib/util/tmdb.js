...
async function getDetails(type, tmdbId, tmdbAPIKey, season) {
  ...
  // Holy fuck TMDB API is inconsistent
  var translation_title_key = ""
  if (type == "movie") {
    translation_title_key = "title"
  } else if (type == "series") {
    translation_title_key = "name"
  }

  const title_in_original_language = "original" + translation_title_key

  var alternative_titles_key = ""
  if (type == "movie") {
    alternative_titles_key = "titles"
  } else if (type == "series") {
    alternative_titles_key = "results"
  }
  ...
  var names =
    res['translations']['translations'].map((x) => {
      ...
      var title = x["data"][translation_title_key]
  ...
  res['alternative_titles'][alternative_titles_key].forEach((x) => {
